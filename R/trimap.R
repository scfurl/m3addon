...
trimap<-function(cds, preprocess_method = "PCA", python_home = system("which python", intern = TRUE), num_dims = NULL, module_file=paste(system.file(package = "m3addon"),"trimap.py", sep = "/"),
                  ...
                  verbose = TRUE, weight_adj = 500.0,  return_seq = FALSE)
{
  ...
  data <- reducedDims(cds)[[preprocess_method]][,1:num_dims]
