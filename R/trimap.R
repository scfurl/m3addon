...
trimap<-function(cds, python_home = system("which python", intern = TRUE), module_file=paste(system.file(package = "m3addon"),"trimap.py", sep = "/"),
                  n_dims = 2, n_inliers= 10, n_outliers = 5, n_random = 5, distance = c('euclidean', 'manhattan', 'angular', 'hamming'), 
                  lr = 1000.0, n_iters = 400, knn_tuple = NULL, apply_pca = TRUE, opt_method = c('dbd', 'sd',  'momentum'), 
                  verbose = TRUE, weight_adj = 500.0,  return_seq = FALSE)
{
  ...
  trimap_args<-c(list(data, n_dims, n_inliers, n_outliers, n_random, distance, lr, n_iters, knn_tuple, apply_pca, opt_method,
                                      verbose, weight_adj, return_seq))
  res <- do.call(trimap_fromR, trimap_args)
  return(res)
}
